...
export class ChatGPTService {
  constructor() {
    this.apiKey = localStorage.getItem('openai_api_key') || OPENAI_API_KEY;
...
// React Hook để sử dụng ChatGPT service
export const useChatGPT = () => {
  ...
  const sendMessage = async (message) => {
    ...
    try {
      const response = await chatGPTService.chatWithGPT(message);
      setIsLoading(false);
      return response;
    } catch (err) {
      setError(err.message);
      setIsLoading(false);
      return chatGPTService.getFallbackResponse(message);
    }
  };

  const resetChat = () => {
    chatGPTService.resetConversation();
    setError(null);
  };
  ...
  return {
    ...
    isLoading,
    error,
    isApiKeySet: chatGPTService.isApiKeySet()
  };
}; 